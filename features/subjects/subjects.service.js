...
const getAllSubjects = async () => {
  const result = await query(
    `SELECT * FROM subjects WHERE is_active = true ORDER BY nama_mapel`
  );
  return result.rows;
};
...
const removeClassSubject = async (id) => {
  const result = await query(
...
const createSubject = async (subjectData) => {
  ...
  const result = await query(
    `INSERT INTO subjects (nama_mapel, deskripsi, icon_url)
     VALUES ($1, $2, $3) RETURNING *`,
    [nama_mapel, deskripsi || null, icon_url || null]
  );
...
const updateSubject = async (subjectId, subjectData) => {
  ...
  const result = await query(
    ...
    [nama_mapel, deskripsi, icon_url, is_active, subjectId]
  );
...
const deleteSubject = async (subjectId) => {
  const result = await query(
    `UPDATE subjects SET is_active = false WHERE id = $1 RETURNING *`,
    [subjectId]
  );
  return result.rows[0];
};
